...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
  modifier onlyOwner() {
        require(msg.sender == owner, "The msg.sender must be contract's owner!");
        _;
    }
  ...
  modifier checkValue(uint _sku) {
    ...
    require(msg.value >= _price, "The msg.value is not sufficient to buy this item!");
    _;
  ...
  modifier forSale(uint256 _sku) {
      require(State.ForSale == items[_sku].state, "This items state must be ForSale!");
      _;
  }
  modifier sold(uint256 _sku) {
      require(State.Sold == items[_sku].state, "This items state must be Sold!");
      _;
  }
  modifier shipped(uint256 _sku) {
      require(State.Shipped == items[_sku].state, "This items state must be Shipped!");
      _;
  }
  modifier received(uint256 _sku) {
      require(State.Received == items[_sku].state, "This items state must be Received!");
      _;
  }
